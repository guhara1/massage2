...
const districtBody = `<main><section class="section"><p class="eyebrow">경기 시흥시</p><h1>경기 시흥시 출장마사지 홈타이</h1><p class="lead">시흥시 출장마사지와 홈타이 가능 지역을 행정동 기준으로 선택하세요. 배곧, 정왕, 은행, 신천 등 생활권별 상담 가능 시간과 요금 안내를 확인할 수 있습니다.</p><div class="neighborhood-grid">${dongs.map((dong) => `<a class="neighborhood-link" href="/area/${enc(region)}/${enc(district)}/${enc(dong)}/"><strong>${dong}</strong></a>`).join("")}</div></section></main>`;
write(`out/area/${region}/${district}/index.html`, layout("경기 시흥시 출장마사지 홈타이 | 마사지허브", "경기 시흥시 출장마사지, 홈타이 예약 시간과 관리사 정보, 요금표, 주의사항을 확인하세요.", districtBody));

for (const dong of dongs) {
  const body = `<main><section class="section"><p class="eyebrow">경기 시흥시 ${dong}</p><h1>경기 시흥시 ${dong} 출장마사지 홈타이</h1><p class="lead">${dong} 출장마사지와 홈타이 예약 시간, 신규 회원 할인, 관리사 스타일, 주의사항을 확인하고 전화로 바로 상담하세요.</p><a class="primary-btn" href="tel:05082024683">전화예약</a></section><section class="section"><h2>${dong} 추천 업체</h2><div class="local-shop-grid"><article class="card"><h3>${dong} 프리미엄 홈타이</h3><p class="lead">상담 후 가능 시간, 관리사 배정, 코스 상세, 후불 안내를 확인할 수 있습니다.</p></article><article class="card"><h3>${dong} 힐링 아로마</h3><p class="lead">시흥 생활권 이동 동선을 기준으로 예상 도착 시간과 코스 구성을 안내합니다.</p></article><article class="card"><h3>${dong} VIP 방문케어</h3><p class="lead">예약 전 요금, 코스 범위, 관리사 정보를 상담으로 확인합니다.</p></article></div></section><section class="section"><div class="local-info"><article class="card"><h2>공지사항</h2><p class="lead">예약 전 가능 시간, 코스, 추가 요금을 확인해 주세요.</p></article><article class="card"><h2>업체소개</h2><p class="lead">마사지허브는 지역별 제휴 가능 업체를 정리하는 중개형 안내 플랫폼입니다.</p></article><article class="card"><h2>관리사정보</h2><p class="lead">관리사 성별, 경력, 가능 코스는 상담으로 안내됩니다.</p></article><article class="card"><h2>주의사항</h2><p class="lead">불법 서비스는 등록과 노출을 허용하지 않습니다.</p></article></div></section></main>`;
  write(`out/area/${region}/${district}/${dong}/index.html`, layout(`경기 시흥시 ${dong} 출장마사지 홈타이 | 마사지허브`, `경기 시흥시 ${dong} 출장마사지, 홈타이 예약 시간과 관리사 정보, 주의사항을 확인하세요.`, body));
}
