...
const shopNames = ["프리미엄 케어", "힐링 아로마", "VIP 방문케어", "시그니처 케어"];
...
for (const [region, districts] of Object.entries(neighborhoods)) {
  for (const [district, dongs] of Object.entries(districts)) {
    ...
    if (existsSync(districtFile)) {
      ...
      html = injectCss(html);
      const grid = `<section class="section"><h2>${district} 행정동 선택</h2><p class="lead">${district} 출장마사지 가능 지역을 행정동 기준으로 선택하세요.</p><div class="neighborhood-grid">${dongs.map((dong) => `<a class="neighborhood-link" href="/area/${enc(region)}/${enc(district)}/${enc(dong)}/"><strong>${dong}</strong></a>`).join("")}</div></section>`;
