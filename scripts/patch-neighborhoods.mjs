...
const neighborhoods = {
  ...
  "경기": {
    "수원시": ["장안구", "권선구", "팔달구", "영통구", "인계동", "매탄동", "영통동", "망포동", "광교동", "호매실동"],
...
const css = `<style>
.neighborhood-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-top:22px}.neighborhood-link{border:1px solid #33261b;border-radius:10px;background:linear-gradient(180deg,#151515,#0f0f0f);padding:13px 12px;font-weight:900;color:#fff}.neighborhood-link strong{color:var(--orange)}.local-shop-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.local-shop{border:1px solid var(--line);border-radius:14px;background:var(--panel);padding:20px}.local-shop h3{margin:0 0 10px}.local-shop p{color:var(--muted);line-height:1.65}.local-info{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}@media(max-width:900px){.neighborhood-grid,.local-shop-grid,.local-info{grid-template-columns:1fr}}
</style>`;

function enc(value) { return encodeURIComponent(value); }
function filePath(region, district, neighborhood) {
  return neighborhood ? `out/area/${region}/${district}/${neighborhood}/index.html` : `out/area/${region}/${district}/index.html`;
}
...
function injectCss(html) {
  return html.includes("neighborhood-grid") ? html : html.replace("</head>", `${css}</head>`);
}
...
for (const [region, districts] of Object.entries(neighborhoods)) {
  for (const [district, dongs] of Object.entries(districts)) {
    ...
    if (existsSync(districtFile)) {
      ...
      html = injectCss(html);
      const grid = `<section class="section"><h2>${district} 행정동 선택</h2><p class="lead">${district} 출장마사지와 홈타이 가능 지역을 행정동 기준으로 선택하세요.</p><div class="neighborhood-grid">${dongs.map((dong) => `<a class="neighborhood-link" href="/area/${enc(region)}/${enc(district)}/${enc(dong)}/"><strong>${dong}</strong> 출장마사지</a>`).join("")}</div></section>`;
      if (!html.includes(`${district} 행정동 선택`)) html = html.replace("</main>", `${grid}</main>`);
...
if (existsSync("out/sitemap.xml")) {
  ...
  for (const [region, districts] of Object.entries(neighborhoods)) {
    for (const [district, dongs] of Object.entries(districts)) {
      for (const dong of dongs) urls.push(`<url><loc>${siteUrl}/area/${enc(region)}/${enc(district)}/${enc(dong)}/</loc></url>`);
    }
  }
